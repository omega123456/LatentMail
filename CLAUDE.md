...
- **Use the date library for date work.** Prefer `date-fns` helpers for parsing, formatting, and manipulating dates/times in both production code and tests instead of manual millisecond arithmetic or ad hoc native `Date` parsing/construction when a library helper covers the case.
