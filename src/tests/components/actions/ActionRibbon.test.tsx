...
describe('ActionRibbon', () => {
  ...
  it('shows Not spam instead of Mark as spam while browsing Spam', () => {
    render(
      <ActionRibbon mailboxId="SPAM" unread={false} starred={false} labels={labels} {...baseHandlers()} />,
    );
  ...
  it('renders only the read toggle and delete in Drafts', () => {
    render(
      <ActionRibbon mailboxId="DRAFT" unread={false} starred={false} labels={labels} {...baseHandlers()} />,
    );
  ...
  it('hides star and delete in Trash', () => {
    render(
      <ActionRibbon mailboxId="TRASH" unread={false} starred={false} labels={labels} {...baseHandlers()} />,
    );
    expect(screen.queryByRole('button', { name: 'Star' })).not.toBeInTheDocument();
  ...
  it('collapses secondary actions into an overflow menu below the width threshold, keeping read/unread and delete outside it', async () => {
    const user = userEvent.setup();
    const handlers = baseHandlers();
