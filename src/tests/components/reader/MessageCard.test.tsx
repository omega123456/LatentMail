...
const message = {
  id: 'message-1', sender: { name: 'Alex', address: 'alex@example.com' }, recipients: [],
  sentAt: new Date('2026-08-10T09:00:00Z'), snippet: 'Preview', html: null, text: null,
};

describe('MessageCard', () => {
  it('fetches a newly opened lazy body once and exposes loading and retry states', async () => {
    const onFetchBody = vi.fn();
    const { rerender } = render(<MessageCard message={{ ...message, htmlPresence: 'neverFetched' }} expanded newest onFetchBody={onFetchBody} loadingBody />);
    expect(onFetchBody).toHaveBeenCalledWith('message-1');
    expect(screen.getByText('Loading message…')).toBeInTheDocument();
    rerender(<MessageCard message={{ ...message, htmlPresence: 'neverFetched' }} expanded newest onFetchBody={onFetchBody} bodyError />);
  ...
  it('expands an older message and shows blocked-image notice', async () => {
    const user = userEvent.setup();
    render(<MessageCard message={{ ...message, html: '<p>Body</p>', htmlPresence: 'present', remoteImagesBlocked: true }} expanded={false} newest={false} />);
