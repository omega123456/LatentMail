...
// ---------------------------------------------------------------------------
// Types — re-exported from queue-types.ts for backward compatibility.
// New code should import directly from './queue-types'.
// ---------------------------------------------------------------------------
...
export type {
  ...
  QueueItem,
  QueueItemSnapshot,
  MailFolderUpdatedPayload,
  MailFetchOlderDonePayload,
} from './queue-types';
    ...
    if (
      ...
      try {
        ...
        if (emailsNeedingBodies.length > 0) {
          ...
          BodyFetchQueueService.getInstance().enqueue(
            item.accountId,
            emailsNeedingBodies.map((email) => ({ xGmMsgId: email.xGmMsgId, xGmThrid: email.xGmThrid })),
