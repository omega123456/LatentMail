...
fn deleting_a_draft_thread_through_triage_never_batch_modifies_it() {
    ...
    runtime.block_on(async {
        ...
        Mock::given(method("POST")).and(path("/token"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({ "access_token": "fresh", "token_type": "Bearer" })))
            .mount(&server).await;
        ...
        Mock::given(method("GET")).and(path("/users/me/drafts"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "drafts": [{ "id": "draft-99", "message": { "id": "message-draft" } }]
            })))
            .mount(&server).await;
        Mock::given(method("DELETE")).and(path("/users/me/drafts/draft-99"))
            .respond_with(ResponseTemplate::new(204)).expect(1).mount(&server).await;
        std::env::set_var("LATENTMAIL_GOOGLE_CLIENT_ID", "client");
        std::env::set_var("LATENTMAIL_GOOGLE_TOKEN_URL", format!("{}/token", server.uri()));
        ...
        AccountRepository::upsert(&connection, &Account {
            id: "account-draft".into(), email: "draft@example.com".into(), display_name: "Draft".into(),
            avatar_url: None, history_id: None, needs_reauthentication: false, created_at: 0, updated_at: 0,
        }).unwrap();
        LabelRepository::ensure_placeholder(&connection, "account-draft", "DRAFT").unwrap();
        MessageRepository::write_full_state(&connection, &Message {
            account_id: "account-draft".into(), id: "message-draft".into(), thread_id: "thread-draft".into(),
            rfc_message_id: None, sender: "A".into(), recipients: "B".into(), subject: "Draft".into(), sent_at: 0,
            snippet: String::new(), html_body: None, plain_body: None, has_attachments: false,
            is_unread: false, is_starred: false, history_id: 1, truncated_body: None, html_presence: HtmlPresence::Absent,
        }).unwrap();
        MessageRepository::set_label_membership(&connection, "account-draft", "message-draft", "DRAFT", true).unwrap();
        drop(connection);
...
fn a_cached_draft_id_is_reused_without_re_listing_drafts() {
    ...
    runtime.block_on(async {
        ...
        Mock::given(method("DELETE")).and(path("/users/me/drafts/cached-draft-id"))
            .respond_with(ResponseTemplate::new(204)).expect(1).mount(&server).await;
        std::env::set_var("LATENTMAIL_GOOGLE_CLIENT_ID", "client");
        std::env::set_var("LATENTMAIL_GOOGLE_TOKEN_URL", format!("{}/token", server.uri()));
        std::env::set_var("LATENTMAIL_GMAIL_BASE_URL", server.uri());
        ...
        let connection = storage.connection().unwrap();
        AccountRepository::upsert(&connection, &Account {
            id: "account-cached-draft".into(), email: "draft@example.com".into(), display_name: "Draft".into(),
            avatar_url: None, history_id: None, needs_reauthentication: false, created_at: 0, updated_at: 0,
        }).unwrap();
        MessageRepository::write_full_state(&connection, &Message {
            account_id: "account-cached-draft".into(), id: "message-cached".into(), thread_id: "thread-cached".into(),
            rfc_message_id: None, sender: "A".into(), recipients: "B".into(), subject: "Draft".into(), sent_at: 0,
            snippet: String::new(), html_body: None, plain_body: None, has_attachments: false,
            is_unread: false, is_starred: false, history_id: 1, truncated_body: None, html_presence: HtmlPresence::Absent,
        }).unwrap();
        MessageRepository::set_draft_id(&connection, "account-cached-draft", "message-cached", "cached-draft-id").unwrap();
        drop(connection);
